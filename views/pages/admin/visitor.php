...
$SUPPORTED = ["MACOS","WINDOWS","LINUX","ANDROID","CHROMEOS","IPHONE"];
...
<div class="content">
    ...
    <div class="title"> <?= LANG[''] ?? "" ?> <span class="sp"> <?= $size ?? '0' ?> <?= LANG[''] ?? "Utilisateur(s)" ?> </span> </div><br>
    ...
    <table>
        ...
        <tbody>
        <?php $i=$size; if(!empty($USERS)){ $IP[]='0zdf'; foreach($USERS as $U){ $country = str_replace(' ','', substr($U['GEO'],2,3) ); ?>

            <tr>
            <td class="<?= !empty($U['BLOCKED']) && $U['BLOCKED'] == 'true' ? "sp" : "" ?>" > <a href="<?= LINK['ADMIN/VISITOR'] ?? "" ?>/block/<?= $U['IP'] ?? "" ?>"> <?= !empty($U['BLOCKED']) && $U['BLOCKED'] == 'true' ? LANG[''] ?? "Débloquer" : LANG[''] ?? "Bloquer" ?> </a> </td>
            <td class="<?php if( in_array($U['IP'],$IP) == false ){ echo "sp"; } ?>" > <?= $U['IP'] ?? "" ?> </td>
            <td> <a href="<?= 'https://'.$_SERVER['SERVER_NAME'].($U['URLS'] ?? "") ?>" target="_blank" > <?= !empty($U['PAGES']) ? $U['PAGES'] : "principale" ?> </a> </td>
            <td class=" green"> <?= $root->datetime_compare($U['DATETIMES'] ?? "") ?> </td>
            <td> <?= $ICON[ in_array(strtoupper($U['OS'] ?? ""),$SUPPORTED) ? strtoupper($U['OS'] ?? "") : "INCONNUE" ] ?? "Inconnue" ?> </td>
            <td> <?= $U['OS'] ?? "" ?> </td>
            ...
            <td> <?= $U['GEO'] ?? "" ?> </td>
            <td> <a href="https://www.google.com/maps/dir//<?= $U['LOC'] ?? "" ?>/@<?= $U['LOC'] ?? "" ?>?hl=fr-FR" target="_blank" > <?= $U['LOC'] ?? "" ?> </a> </td>
            <td> <?= $U['TIMEZONES'] ?? "" ?> </td>
            <td> <?= $U['ORG'] ?? "" ?> </td>
            <td> <?= $U['DEVISE'] ?? "" ?> </td>
        ...
        <?php $IP[] = $U['IP']; } }else{ ?>
            <tr>
            <td colspan="8" class="sp"> <?= LANG[''] ?? "No user(s)" ?> </td>
            </tr>
        <?php } ?>
        </tbody>
    </table>
